# container-github-action-galaxy

A container that is used for [GitHub actions galaxy](https://github.com/ucomesdag/galaxy-action).

[![github-action-molecule build status][quai.io]](https://quay.io/repository/ucomesdag/github-action-galaxy?tab=tags)

This container contains:
 - ansible-galaxy

<!-- container image -->

[quai.io]: https://img.shields.io/badge/Quay.io-container%20image-green?style=flat&logo=data:image/png;base64,&logoColor=white
